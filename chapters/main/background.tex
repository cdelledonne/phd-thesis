...
technological progress. The last two challenges may sound similar --- they are both arising from the
need for information from a remote node. However, the fourth challenge describes a distributed
scheduling problem, whereas the fifth encapsulates two local requirements --- \acrshort{os}-level
locking mechanisms and ``good'' quantum memories. It is the last two challenges that fundamentally
differentiate a quantum networking node from a quantum computing system, and are the key driver for
a \emph{networked} quantum node operating system.
