...
technological progress. These last two challenges are both arising from the need for information
from a remote node at a certain level and with a certain degree of precision. However, the fourth
challenge describes a distributed scheduling problem, whereas the fifth encapsulates two local
requirements --- \acrshort{os}-level locking mechanisms and ``good'' quantum memories. It is the
last two challenges that fundamentally differentiate a quantum networking node from a quantum
